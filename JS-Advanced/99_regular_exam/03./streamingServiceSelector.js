export const streamingServiceSelector = {
    ...
    availablePlatforms(platforms, selectedPlatformIndex) {
        if (!Array.isArray(platforms) || !Number.isInteger(selectedPlatformIndex) || selectedPlatformIndex < 0 || selectedPlatformIndex >= platforms.length) {
    ...
    contentRating(runtimeInMinutes, viewerRating) {
        ...
        if (typeof runtimeInMinutes !== 'number' || runtimeInMinutes <= 0 || typeof viewerRating !== 'number' || viewerRating < 0 || viewerRating > 10) {
...
// console.log(streamingServiceSelector.availablePlatforms('Movie', 'Netflix', 'first'));
